...
const WS_URL = __DEV__ ? "http://localhost:3001" : "https://api.evn.app";
...
interface SyncState {
  ...
  // Event listeners
  onExpenseCreated: (
    callback: SyncEventCallback<ExpenseCreatedEvent>,
  ) => () => void;
  onExpenseUpdated: (
    callback: SyncEventCallback<ExpenseUpdatedEvent>,
  ) => () => void;
  onExpenseDeleted: (
    callback: SyncEventCallback<ExpenseDeletedEvent>,
  ) => () => void;
  onSettlementCreated: (
    callback: SyncEventCallback<SettlementCreatedEvent>,
  ) => () => void;
  onBalancesUpdated: (
    callback: SyncEventCallback<BalancesUpdatedEvent>,
  ) => () => void;
}
...
export const useSyncStore = create<SyncState>((set, get) => ({
  ...
  connect: () => {
    ...
    newSocket.on("welcome", (data: { message: string; socketId: string }) => {
      logger.debug("Welcome from server", {
        category: "sync",
  ...
  joinGroup: async (groupId: string): Promise<boolean> => {
    ...
    if (!socket?.connected) {
      logger.warn("Cannot join group - not connected", {
        category: "sync",
        groupId,
      });
      return false;
  ...
  leaveGroup: async (groupId: string): Promise<boolean> => {
    ...
    if (!socket?.connected) {
      logger.warn("Cannot leave group - not connected", {
        category: "sync",
        groupId,
      });
      return false;
