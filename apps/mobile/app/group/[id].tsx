...
export default function GroupDetailScreen() {
  ...
  useEffect(() => {
    ...
    const unsubExpenseCreated = onExpenseCreated((data) => {
      if (data.expense.groupId === id) {
        queryClient.invalidateQueries({
          queryKey: queryKeys.expenses.byGroup(id),
        });
        queryClient.invalidateQueries({
          queryKey: queryKeys.groups.balances(id),
        });
      }
    });
    ...
    const unsubSettlementCreated = onSettlementCreated((data) => {
      if (data.settlement.groupId === id) {
        queryClient.invalidateQueries({
          queryKey: queryKeys.settlements.byGroup(id),
        });
        queryClient.invalidateQueries({
          queryKey: queryKeys.groups.balances(id),
        });
    ...
    return (
      ...
      >
        ...
        <View className="flex-1 justify-center items-center px-8">
          ...
          <PrimaryButton onPress={refetchAll}>Try Again</PrimaryButton>
        </View>
      </SafeAreaView>
    );
  }
  ...
  return (
    ...
    >
      ...
      {/* Balance Summary */}
      <View
        className={`items-center py-6 border-b ${isDark ? "border-dark-border" : "border-light-border"}`}
      >
        <Text
      ...
      {/* Tabs */}
      <View
        className={`flex-row border-b ${isDark ? "border-dark-border" : "border-light-border"}`}
      >
        {(["expenses", "balances", "members"] as const).map((tab) => (
          <Pressable
            key={tab}
            className={`flex-1 py-3 items-center active:opacity-70 ${
              activeTab === tab ? "border-b-2 border-dutch-orange" : ""
            }`}
