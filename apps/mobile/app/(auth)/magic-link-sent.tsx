  ...
  return (
    <SafeAreaView className={`flex-1 ${isDark ? "bg-dark-bg" : "bg-light-bg"}`}>
      <View className="flex-1 p-6 justify-center items-center">
        <View
          className={`w-24 h-24 rounded-full items-center justify-center mb-8 ${isDark ? "bg-dutch-blue/20" : "bg-dutch-blue/10"}`}
        >
          <Text className="text-5xl">✉️</Text>
        </View>

        <Text
          className={`text-2xl font-bold mb-4 text-center ${isDark ? "text-white" : "text-black"}`}
        >
          Check your email
        </Text>

        <Text
          className={`text-base text-center leading-6 mb-6 ${isDark ? "text-dark-text-secondary" : "text-light-text-secondary"}`}
        >
          We sent a magic link to{"\n"}
          <Text className="font-semibold text-dutch-blue">
            {magicLinkEmail}
          </Text>
        </Text>

        <Text
          className={`text-sm text-center leading-5 mb-8 ${isDark ? "text-dark-text-tertiary" : "text-light-text-tertiary"}`}
        >
