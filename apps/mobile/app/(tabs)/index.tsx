...
export default function GroupsScreen() {
      ...
      <Link href={`/group/${item.id}`} asChild>
        ...
        >
          ...
          <View className="flex-1 ml-3">
            ...
            <Text
              className={`text-sm mt-0.5 ${isDark ? "text-dark-text-secondary" : "text-light-text-secondary"}`}
            >
              {item.defaultCurrency}
            </Text>
          </View>
          <Text
            className={`text-lg ${isDark ? "text-dark-text-tertiary" : "text-light-text-tertiary"}`}
          >
            {">"}
          </Text>
        </Pressable>
  ...
  const getItemLayout = useCallback(
    (_: ArrayLike<Group> | null | undefined, index: number) => ({
        ...
        options={{
          ...
          headerRight: () => (
            <View className="flex-row items-center gap-3">
              ...
              >
                <Text className="text-dutch-orange text-base font-medium">
                  Join
                </Text>
              </Pressable>
              <Pressable
      ...
      {groups.length === 0 ? (
        <View className="flex-1 justify-center items-center px-8">
          ...
          <View className="w-full gap-3">
            ...
            <GhostButton onPress={handleJoinGroup}>Join with Code</GhostButton>
          </View>
        </View>
      ) : (
        <FlatList
