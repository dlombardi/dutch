...
/**
 * Pending expense waiting to be synced when back online.
 */
...
interface OfflineQueueState {
  ...
  // Actions
  queueExpense: (expense: Omit<PendingExpense, 'localId' | 'createdAt'>) => PendingExpense;
  ...
  persist(
    (set, get) => ({
      ...
      removePendingExpense: (localId) => {
        set((state) => ({
          pendingExpenses: state.pendingExpenses.filter((p) => p.localId !== localId),
        }));
      },
      ...
      getFailedPendingExpenses: () => {
        return get().pendingExpenses.filter((p) => (p.retryCount ?? 0) >= MAX_RETRY_COUNT);
      },
    }),
    {
